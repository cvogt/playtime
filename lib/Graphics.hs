...
computeSpritePlacements :: (TextureId -> Texture) -> GameState -> [TexturePlacements]
computeSpritePlacements textures (GameState GenericGameState {..} TransientGameState {..} PersistentGameState {..}) =
  sprites <> highlightMouserOver
  where
    sprites =
      tiles
