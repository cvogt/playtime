...
stepGameStatePure randInts EngineState {..} gs@GameState {..} = \case
  KeyEvent Key'Space KeyState'Pressed ->
    let Dimensions {width} = gsLogicalDimensions
  ...
  RenderEvent _ ->
    let velocity = 200
        step = gsTimePassed * velocity
        Dimensions {height, width} = gsLogicalDimensions
        moveX = if Key'A `setMember` gsKeysPressed then subtract step else if Key'D `setMember` gsKeysPressed then (+ step) else id
        moveY = if Key'W `setMember` gsKeysPressed then subtract step else if Key'S `setMember` gsKeysPressed then (+ step) else id
        randDoubles = int2Double . (`mod` double2Int height) <$> randInts
        bulletVelocityX = 200
        killed = catMaybes $ do
          bulletArea <- join $ gsBullets <&> \pos -> flip Area 100 <$> trajectoryPixels pos gsTimePassed bulletVelocityX 0
          enemy <- gsEnemies
     ...
     in gs
          { gsPlayer = updateY moveY $ updateX moveX gsPlayer,
            gsEnemies =
              updateX (subtract $ 50 * gsTimePassed) . updateX (`mod'` width) <$> newEnemies',
            gsBullets =
              updateX (+ bulletVelocityX * gsTimePassed) <$> filter ((< 1024) . x) gsBullets,
            gsStars =
              updateX (subtract $ 5 * gsTimePassed) . updateX (`mod'` width) <$> gsStars
          }
  _ -> gs
